...
export const createProduct = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!name || !description || !price || !category || !stock) {
    return next(new ErrorHandler("Please provide all products details.", 400));
  }
...
export const fetchAllProducts = catchAsyncErrors(async (req, res, next) => {
  ...
  const newProductsResut = await database.query(newProductsQuery);
  ...
  res.status(200).json({
    ...
    totalProducts,
    newProducts: newProductsResut.rows,
...
export const postProductReview = catchAsyncErrors(async (req, res, next) => {
  ...
  if (rows.length === 0) {
    return res.status(403).json({
      success: false,
      message: "You can only review a product you have purchase",
    });
  }
